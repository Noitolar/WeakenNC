...
# id = 0
# PATH_TO_INFO = os.path.join(os.getcwd(), 'model_weights/') + '_sota/'
...
PATH_TO_INFO = args.path + "info.pkl"
out_path = os.path.join(os.path.dirname(PATH_TO_INFO), 'imgs/')
if not os.path.exists(out_path):
    os.makedirs(out_path, exist_ok=True)
...
def plot_ETF():
    ...
    # -------------------------------------- plot for figure 6 ---------------------------------------------------------
    ETF = info['ETF_metric']
    plt.plot(ETF, 'c', marker='v', ms=16, markevery=25, linewidth=5, alpha=0.7)
    plt.xlabel('Epoch', fontsize=40)
