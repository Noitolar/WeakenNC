...
def trainer(args, model, trainloader, epoch_id, criterion, optimizer, scheduler, logfile):
    ...
    print_and_save('\nTraining Epoch: [%04d | %04d] LR: %f' % (epoch_id + 1, args.epochs, scheduler.get_last_lr()[-1]), logfile)
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        if batch_idx % 10 == 0:
            print_and_save('[epoch: %04d] (%04d/%04d) | Loss: %.4f | top1: %.4f | top5: %.4f ' %
...
def main():
    ...
    if args.model == "MLP":
        ...
        model = models.__dict__[args.model](num_classes=num_classes, fc_bias=args.bias, ETF_fc=args.ETF_fc, fixdim=args.fixdim, SOTA=args.SOTA, weaken=args.weaken).to(device)
